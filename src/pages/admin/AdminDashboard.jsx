...
import { IconUsers, IconPill, IconClipboardList, IconAlertTriangle } from '@tabler/icons-react'
  ...
  return (
    <Layout>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px,1fr))', gap: '1rem', marginBottom: '1.8rem' }}>
        <StatCard label="Pending approvals" value={stats?.pendingUsers}  icon={<IconUsers          size={24} stroke={1.5} />} accent="#fef3c7" />
        <StatCard label="Total products"    value={stats?.totalProducts} icon={<IconPill           size={24} stroke={1.5} />} accent="#d8f3dc" />
        <StatCard label="Scans logged"      value={stats?.totalScans}    icon={<IconClipboardList  size={24} stroke={1.5} />} accent="#dbeafe" />
        <StatCard label="Open reports"      value={stats?.openReports}   icon={<IconAlertTriangle  size={24} stroke={1.5} />} accent="#fee2e2" />
      </div>
